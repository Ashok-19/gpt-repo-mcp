...
- `workspace_run_script` is the preferred tool for quick experiments because inline script text is first materialized under `scratch/agents/<agent_id>/workspace-runs/`. `workspace_run_python` and `workspace_run_bash` expose the same scratch-backed flow as focused aliases.

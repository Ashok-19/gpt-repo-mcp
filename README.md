...
- `workspace_exec` runs direct argv commands, including `uv run`, from an explicit repo-relative cwd. `workspace_run_script` covers inline Python, Node, and POSIX snippets; successful wrappers are removed automatically and failed wrappers are retained for diagnosis.
